...
/* =====================================================
   ...
   إعداد: إيمان الزهراني + مساعد التطوير الذكي
   ===================================================== */
...
function makeQuestion(pool, unit, level){
  ...
  let baseVal = (level === 1) ? rnd(2,900)/10 :
                (level === 2) ? rnd(5,900) :
                                 rnd(50,5000);
...
function loadLevel(){
  ...
  startTimer(timeByLevel[Game.level-1]);
  askNext();
}
...
function askNext(){
  ...
  if(!q){
    endLevel(true, "أحسنت! أنهيت جميع الأسئلة.");
    return;
  }
...
function handleAnswer(btn, q){
  ...
  if(btn.textContent === q.answer){
    ...
    ui.feedback.innerHTML = '❌ ليست صحيحة — حاول مجددًا<br>' + q.explain;
